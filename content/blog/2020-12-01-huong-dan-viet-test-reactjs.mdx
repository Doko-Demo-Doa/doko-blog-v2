---
title: (VN) Hướng dẫn viết test ReactJS cho người mới bắt đầu
description: Hướng dẫn viết unit test với ReactJS
author: Kuon
date: 2020-12-01
...
![react-testing](/img/react-testing/react-testing-library.png)

Disclaimer: 1 số lib / tool trong bài có thể đã đã cũ, nhưng tư tưởng chung của bài viết vẫn còn giá trị tham khảo.
...
Về React nói riêng, khi nhắc đến test là không thể không nhắc đến **jest** và **@testing-library/react** (hay _Testing Library_). Có rất nhiều thư viện / framework test cho JS hiện nay. Ta chọn **Jest** là công cụ thay thế **Mocha**, **Ava**, **Jasmine**. Còn _Testing Library_ là lựa chọn khác thay thế cho _Enzyme_ của _airbnb_ (đang được rất nhiều người sử dụng).

_Testing Library_ tiếp cận việc test dưới góc độ của người dùng. Do đó nó sẽ dẫn bạn đến việc viết integration test, là lúc mà nhiều component được test đồng thời cùng nhau.
...
![reddit](/img/react-testing/react-testing-intro-2.gif)
...
Form trên sẽ lưu giá trị của nó trong một biến state hook. Khi click nút thì prop `onSearch` truyền vào từ component cha sẽ được gọi ra.
...
Khi bạn đã hiểu được "tư tưởng" của _Testing Library_, việc test không còn bí ấn hay đáng sợ nữa.

> **Ghi chú:** Một người dùng ở đây có thể là người dùng cuối, hoặc cũng có thể là một developer khác đang dùng component mà bạn đã viết. Ví dụ như khi bạn viết một component hiện ảnh dạng gallery cho một dev cùng team. Cái ta cần test là gallery đã viết ra hoạt động đúng khi prop hay đổi chẳng hạn.
...
Nào, bắt đầu với phần header trước. Đầu tiên ta mở file `src/App.test.js` và bỏ phần code test cũ đi. Sau đó ta viết định nghĩa cho phần test bằng hàm `describe(...)` của Jest.

> **Ghi chú:** Việc đóng code test vào `describe` không bắt buộc, nhưng nhờ nó chúng ta sẽ có thể nhóm các phần test lại khi chạy, và gộp lại cho dễ nhìn trong editor.

```jsx
describe("Header", () => {});
```

Test case được khai báo bằng từ `test(...)` hoặc `it(...)`. Cả 2 cái này đều có trong Jest.
...
Đại khái là cái `App.tsx` trông như này:
...
Để ý thấy là không có Router ở đây. Vì mục đích test, ta sẽ render nó ra ngoài App trong file `index.js` mà ta sẽ [bọc cái app trong một MemoryRouter](https://reactrouter.com/web/guides/testing).

Do đó trong bước đầu tiên, ta sẽ render App component. _Testing Library_ có cung cấp sẵn cho ta một hàm `render` để tạo ra DOM cho một component đã cho trước.
...
Vì app của chúng ta tạo từ `create-react-app`, mọi thứ cần và đủ cho _Testing Library_ cũng đã đều được cài và thiết lập ổn từ đầu.
...
Đây chính là lúc chúng ta cần đến `debug`. Nó sẽ giúp chúng ta in ra DOM tree ở mọi lúc mà ta muốn. Đương nhiên là nhìn không đẹp và tiện như Dev Tools nhưng nó đã giúp cho ta thấy điều gì xảy ra bên trong những dòng test.
...
Nói đi nói lại là cuối cùng chúng ta nên dùng hàm `debug`:
...
![debug](/img/react-testing/testing-debug.png)

OK, dễ thấy là header đang chứa vài link trong đó, bao gồm cả phần "How it works". Giờ chung ta sẽ tìm hiểu cách để _truy nhập_ và _tương tác_ với nó.

## Làm sao để truy cập DOM tree
...
- **getBy\*** như `getByTestId`, `getByText`, hay `getByRole`. Những hàm này đều là các hàm đồng bộ (synchronous) và dùng để kiểm tra xem element đang có trong DOM hay không, nếu không thì bắn ra lỗi.

- **findBy\*** như `findByText`: Các hàm này là các hàm bất đồng bộ. Chúng sẽ đợi một khoảng thời gian (mặc định là 5s) cho đến khi element xuất hiện trong DOM, nếu không thì bắn ra lỗi.

- **queryBy**: Cũng giống getBy nhưng khi không tìm thấy thì không bắn ra lỗi, mà chỉ trả về null.

Như có thể thấy, ta đã có ngay cơ số lựa chọn, mà trên đây mới [chỉ là một số nhỏ trong danh sách đầy đủ](https://testing-library.com/docs/dom-testing-library/api-queries) mà thôi. Câu hỏi là: Ta nên dùng cái nào?

Thoạt nhìn thì có vẻ `getByTestId` là chuẩn bài. Chúng ta chỉ cần thêm test id vào element như sau:

```jsx
<div data-testid="some-content">Some content</div>
```

Bây giờ chúng ta có thể truy cập vào cái div kia bằng hàm `getByTestId('some-content')`. Rất đơn giản, phải không?

Nhưng rõ ràng là ta đang phải sửa code chỉ để pass test, không tốt, không tốt. Liệu có cách nào tốt hơn?

_Testing Library_ có hệ thống tài liệu khá ngon và đáng đọc. Và trong đó có phần tài liệu mô tả [truy vấn nào nên sử dụng trong trường hợp nào](https://testing-library.com/docs/guide-which-query).

Các query ngon nhất mà mọi người đều dùng được sẽ có ưu tiên cao hơn. Trong số đó `getByRole` sẽ là lựa chọn của chúng ta. `getByAltText` và `getByTitle` sẽ dược dùng trong một số trường hợp đặc biệt hoặc ngoại lệ, và `getByTestId` được xếp cuối. Chúng ta sẽ chỉ test theo ID khi không còn lựa chọn nào khác.

Nào, vậy ta sẽ thử `getByRole`. Tham số đầu tiên sẽ là role aria của element. Ở đây thì nó là `link`. Do có nhiều hơn một link trên page, ta sẽ phải chỉ định tên của element bằng tuỳ chọn `name`.
...
Ở đây chúng ta dùng dạng biểu thức chính quy (regular expression): `/how it works/` thay vì viết dạng string kiểu `'How it works'`. Cách này sẽ giúp ta tránh được vấn đề về chữ hoa chữ thường (có thể bị gây ra sau khi áp CSS vào). Ngoài ra chúng ta cũng có thể lấy một phần string. `/how it/i` sẽ pass test, còn `'How it'` thì không.

Lưu lại và chạy thử, test sẽ pass, nghĩa là chúng ta có tìm thấy link.

Vì mới bắt đầu, tại sao ta không thử kiểm tra lại một lần nữa xem sao? Bạn còn nhớ function `debug` phía trên chứ? Nó có thể nhận thêm 1 tham số đầu vào, bằng cách này bạn có thể đưa một element đơn vào trong console để in ra:

![debug element](/img/react-testing/debugging.png)

## Tương tác với các phần tử trong DOM
...
- Dùng fireEvent.click có trong `@testing-library/react`
- Dùng function click có trong `@testing-library/user-event`
...
## Test xem trang muốn test có được render đúng chưa
...
![link navigation](/img/react-testing/react-testing-intro-link.gif)

Sau khi click vào link thì mong muốn là thấy một trang có chữ "How it works".

Nếu trên headline có Aria role, chúng ta có thể dùng lại **getByRole** một lần nữa để kiểm tra xem có đang ở trang mong muốn không. Theo tài liệu [MDN](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/ARIA_Techniques#Roles) thì nó có: `heading`.
...
<img src="/img/react-testing/react-testing-intro-form-1.gif" alt="" />
...
## Tránh trùng lặp với setup function
...
## Thay đổi dữ liệu và submit form
...
<img src="/img/react-testing/3-debug-form.png" alt="" />
...
<img src="/img/react-testing/5-debug-number-of-top-posts.png" alt="" />
...
<img src="/img/react-testing/6-copy-response.png" alt="" />
...
<Callout type="warning">
  Khi ứng dụng cần gửi nhiều API request, cách mock kiểu này sẽ gây ra phiền
  toái kha khá. Lúc đó bạn nên xem xét việc sử dụng package
  [này](https://github.com/mswjs/msw). Thông tin thêm các bạn có thể tìm ở
  [đây](https://kentcdodds.com/blog/stop-mocking-fetch).
</Callout>
...
<Callout type="warning">
  Ghi chú: Trong thời gian làm việc với testing, có thể bạn sẽ cần mock các hàm
  với nhiều công dụng khác nhau. Với jest bạn có thể sử dụng luôn `jest.fn()`,
  bản thân `jest-mock-fetch` cũng dùng nó đấy!
</Callout>
